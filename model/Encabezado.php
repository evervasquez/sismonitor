...
class Encabezado extends Main {
    ...
    public function save_cabecera_1($p) {
        ...
        $institucion->inst_id = $a['codigo_modular']; //n
        $inst_id = $a['codigo_modular'];
        $institucion->idgrupo_intervencion = $a['grup_intervencion']; //s
        $institucion->inst_nro_escuela = $a['num_escuela']; //s
        $institucion->inst_nombre = $a['nombre_escuela']; //s
        ...
        $institucion->inst_ma = $a['turno_manana_fin']; //s
        $institucion->inst_td = $a['turno_tarde_ini']; //s
        $institucion->inst_ta = $a['turno_tarde_fin']; //s
        $institucion->inst_dd = $a['duracion_desayuno_ini']; //s
        $institucion->inst_da = $a['duracion_desayuno_fin']; //s
        $institucion->inst_rd = $a['duracion_recreo_ini']; //s
        $institucion->inst_ra = $a['duracion_recreo_fin']; //s
        $institucion->inst_ad = $a['duracion_almuerzo_ini']; //s
        $institucion->inst_aa = $a['duracion_almuerzo_fin']; //s

        $institucion->inst_datosubigeo = $a['punto_referencial']; //s
        $institucion->inst_distkm = $a['enca_distancia']; //s
        $institucion->inst_tiempomin = $a['tiempo_minutos']; //s
        $institucion->forma_transporteid = $a['form_transporte']; //s
        $institucion->estado = $a['inst_estado']; //s
        ...
        $institucion->inst_distribucion_aulas = $aulas_grados; //s
        $institucion->inst_nro_aulas = count($idx_); //s
        $institucion->inst_nro_asociados = $a['numero_padres_t']; //s
        //***
        $institucion->nro_padres = $a['numero_padres']; //s
        $institucion->nro_padres_d = $a['numero_padres_d']; //s
        $institucion->nro_madres = $a['numero_madres']; //s
        $institucion->nro_madres_d = $a['numero_madres_d']; //s
        //**
        $institucion->inst_conei_rec_ugel = bool_($p['conei_cuenta'][1]['value']); //s
        $institucion->inst_conei_res_direc = bool_($p['conei_cuenta'][2]['value']); //s
        $institucion->inst_pei = bool_($p['conei_cuenta'][3]['value']); //s
        $institucion->inst_pei_a = bool_($p['conei_cuenta'][4]['value']); //s
        //$institucion->inst_vision=bool_($p['conei_cuenta'][4]['value']);//s
        //$institucion->inst_mision=bool_($p['conei_cuenta'][5]['value']);//s
        $institucion->inst_ident = bool_($p['conei_cuenta'][5]['value']); //s
        $institucion->inst_diagnostico = bool_($p['conei_cuenta'][6]['value']); //s
        $institucion->inst_prop_pedagogica = bool_($p['conei_cuenta'][7]['value']); //s
        $institucion->inst_prop_gestion = bool_($p['conei_cuenta'][8]['value']); //s
        //
        $institucion->inst_pci = bool_($p['conei_cuenta'][9]['value']); //s
        $institucion->inst_pci_solo = bool_($p['conei_cuenta'][10]['value']); //s
        //
        $institucion->inst_pat = bool_($p['conei_cuenta'][11]['value']); //s
        $institucion->inst_pat_solo = bool_($p['conei_cuenta'][12]['value']); //s

        $institucion->inst_luz_electrica = bool_($p['servicios'][1]['value']); //s
        $institucion->inst_agua_potable = bool_($p['servicios'][2]['value']); //s
        $institucion->inst_desague = bool_($p['servicios'][3]['value']); //s
        $institucion->inst_relleno_sanitario = bool_($p['servicios'][4]['value']); //s
        $institucion->inst_sshh_pozo_septico = bool_($p['servicios'][5]['value']); //s
        $institucion->inst_sshh_letrina_silo = bool_($p['servicios'][6]['value']); //s
        $institucion->inst_telefono = bool_($p['servicios'][7]['value']); //s
        $institucion->save();
        //

        $autoridades = ORM::for_table('autoridades')->create();

        $autoridades->inst_id = $inst_id;
        $autoridades->gob_cd_h = $a["autoridades_1_1"];
        $autoridades->gob_sd_h = $a["autoridades_1_2"];
        $autoridades->ten_cd_h = $a["autoridades_1_3"];
        $autoridades->ten_sd_h = $a["autoridades_1_4"];
        $autoridades->age_cd_h = $a["autoridades_1_5"];
        $autoridades->age_sd_h = $a["autoridades_1_6"];
        $autoridades->otro_cd_h = $a["autoridades_1_7"];
        $autoridades->otro_sd_h = $a["autoridades_1_8"];

        $autoridades->gob_cd_m = $a["autoridades_2_1"];
        $autoridades->gob_sd_m = $a["autoridades_2_2"];
        $autoridades->ten_cd_m = $a["autoridades_2_3"];
        $autoridades->ten_sd_m = $a["autoridades_2_4"];
        $autoridades->age_cd_m = $a["autoridades_2_5"];
        $autoridades->age_sd_m = $a["autoridades_2_6"];
        $autoridades->otro_cd_m = $a["autoridades_2_7"];
        $autoridades->otro_sd_m = $a["autoridades_2_8"];
        $autoridades->save();
        
        for ($j_1 = 1; $j_1 <= 4; $j_1++) {
        $gubernamentales = ORM::for_table('instituciones_gubernamentales')->create();
        $gubernamentales->inst_id = $inst_id;
        $gubernamentales->nombre = $a["gubernamentales1_".$j_1];
        $gubernamentales->guber = $a["gubernamentales_".$j_1];
        $gubernamentales->save();
        }
        ...
        for ($i_1 = 1; $i_1 <= 9; $i_1++) {
            ...
            $ambiente_->inst_id = $inst_id; //n
            $ambiente_->tipo_ambienteid = $i_1; //n
            $ambiente_->tiene = bool_($ambiente[$i_1][1]['value']); //s
            $ambiente_->bueno = $ambiente[$i_1][2]['value']; //s
            $ambiente_->regular = $ambiente[$i_1][3]['value']; //s 
            $ambiente_->malo = $ambiente[$i_1][4]['value'];  //s
            $ambiente_->save();
        }
        ...
        for ($i_1 = 1; $i_1 <= count($poblacion_); $i_1++) {
            ...
            $pobla_->inst_id = $inst_id; //n
            $pobla_->grado = $i_1; //n
            $pobla_->hombres = $poblacion_[$i_1][1]['value'];
            $pobla_->hombres_d = $poblacion_[$i_1][2]['value'];
            //***
            $pobla_->mujeres = $poblacion_[$i_1][4]['value'];
            $pobla_->mujeres_d = $poblacion_[$i_1][5]['value'];
            // cuidado con los numero del array
            //***
            $pobla_->save();
        }
        //echo '-';
        ...
        for ($i_1 = 1; $i_1 <= count($miembros_escolar); $i_1++) {
            if ($miembros_escolar[$i_1][1]['value'] != "") {
                ...
                $miembros_escolar_->inst_id = $inst_id; //n
                $miembros_escolar_->cargo_id = $miembros_escolar[$i_1][0]['value']; //n
                $miembros_escolar_->nombres_apell = $miembros_escolar[$i_1][1]['value']; //s
                $miembros_escolar_->sexo = $miembros_escolar[$i_1][2]['value']; //s
                $miembros_escolar_->edad = $miembros_escolar[$i_1][3]['value']; //s
                $miembros_escolar_->grado = $miembros_escolar[$i_1][4]['value']; //s
                $miembros_escolar_->discapacidad = $miembros_escolar[$i_1][5]['value']; //s
        ...
        for ($i_1 = 1; $i_1 <= count($miembros_conei); $i_1++) {
            ...
            if ($miembros_conei[$i_1][1]['value'] != "") {
                ...
                $miembros_conei_->inst_id = $inst_id; //n
                $miembros_conei_->cargoid = $miembros_conei[$i_1][0]['value']; //n
                $miembros_conei_->nombres = $miembros_conei[$i_1][1]['value']; //s
                $miembros_conei_->sexo = $miembros_conei[$i_1][2]['value']; //s
                $miembros_conei_->dni = $miembros_conei[$i_1][3]['value']; //s
                $miembros_conei_->discapacidad = $miembros_conei[$i_1][4]['value'];
        ...
        for ($i_1 = 1; $i_1 <= count($personal); $i_1++) {
            ...
            if ($personal[$i_1][0] != "") {
                ...
                if ($personal[$i_1][19] != 1) {
                    $personal[$i_1][19] = 0;
                }
                $personal_->pers_es_director = $personal[$i_1][18];
                $personal_->pers_discapacidad = $personal[$i_1][19];
                $personal_->save();
            }
        }
...
//        }
        ...
        $institucion->inst_nro_asociados = $a['numero_padres_t'];
        //**
        $institucion->nro_padres = $a['numero_padres']; //s
        $institucion->nro_padres_d = $a['numero_padres_d']; //s
        $institucion->nro_madres = $a['numero_madres']; //s
        ...
        //
        ORM::delete_many(ORM::for_table('autoridades')->where('inst_id', $a['inst_id'])->find_many());
        //
        ORM::delete_many(ORM::for_table('instituciones_gubernamentales')->where('inst_id', $a['inst_id'])->find_many());
        ...
        for ($i_1 = 1; $i_1 <= count($personal); $i_1++) {
            ...
            if ($personal[$i_1][0] != "") {
                ...
                $personal_->pers_grado_ensena = $grad;
                $personal_->pers_experiencia_director = $personal[$i_1][4];
                $personal_->pers_sexo = $personal[$i_1][7];
                $personal_->pers_experiencia_docente = $personal[$i_1][5];
                $personal_->pers_experiencia_escuela = $personal[$i_1][6];
                $personal_->pers_experiencia_proyecto = $personal[$i_1][3];
                $personal_->pers_estado_civil = $personal[$i_1][8];
                $personal_->pers_condicion_lab = $personal[$i_1][9];
                $personal_->pers_fecha_nac = todate_($personal[$i_1][10]);
                $personal_->pers_dni = $personal[$i_1][11];
                ...
                $personal_->pers_es_director = $personal[$i_1][18];
                $personal_->pers_discapacidad = $personal[$i_1][19];
                $personal_->pers_ano_aprendes = $personal[$i_1][20];
                $personal_->pers_ano_suma = $personal[$i_1][21];
                $personal_->pers_ano_pip = $personal[$i_1][22];
                $personal_->pers_ano_cepco = $personal[$i_1][23];
                $personal_->save();
            }
        }
