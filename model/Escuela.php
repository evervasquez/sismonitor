...
class  Escuela extends Main 
{
    ...
    function index($query , $p ) 
    {
		...
		categoria_escuela.categoria_escuela,
		ubigeo_e.descripcion,
		CONCAT(facilitador.nombres,' ',facilitador.apellidos)
		FROM
		escuela
		INNER JOIN ubigeo_e ON concat(substr(escuela.id_ibugeo, 1,2),'0000') = ubigeo_e.id_ubigeo
		INNER JOIN tipo_escuela_e ON escuela.tipo_escuela_id = tipo_escuela_e.tipo_escuela_id
		INNER JOIN categoria_escuela ON escuela.categoria_escuela_id = categoria_escuela.categoria_escuela_id
		INNER JOIN facilitador ON escuela.facilitador_id = facilitador.facilitador_id
            WHERE escuela LIKE :query OR escuela_id = :id OR tipo_escuela_e.tipo_escuela = :id ORDER BY 1 DESC";
