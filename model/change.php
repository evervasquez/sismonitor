...
$a= new Area();
$lista_="";
$rw=$a->getSelect("SELECT inst_id,inst_distribucion_aulas FROM institucion");
for ($i = 0; $i < count($rw); $i++) {
  $all =explode(';', $rw[$i]['inst_distribucion_aulas']);
  ...
  $a->getSelect("UPDATE institucion SET inst_distribucion_aulas='$lista_' WHERE inst_id={$rw[$i]['inst_id']}");
}
