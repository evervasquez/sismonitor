...
class Evaluacion extends Main
{
    function index($query , $p ) 
    {
        ...
        INNER JOIN institucion ON estudiante.escuela_id = institucion.inst_id
        ...
        WHERE proyecto.proyecto LIKE :query OR area.area LIKE :query OR estudiante.nombre LIKE :query OR estudiante.apellidos LIKE :query OR institucion.inst_nombre LIKE :query OR evaluacion.evaluacion_id = :id GROUP BY evaluacion_id ORDER BY 1 DESC";
    ...
    function mostrar_estudiante($query , $p ) 
    {
		...
		estudiante
		INNER JOIN institucion ON estudiante.escuela_id = institucion.inst_id
        WHERE estudiante.nombre LIKE :query OR estudiante.apellidos LIKE :query OR institucion.inst_nombre LIKE :query OR institucion.inst_nro_escuela LIKE :query OR estudiante.estudiante_id = :id ORDER BY 1 DESC";
        $param = array(array('key'=>':query' , 'value'=>"%$query%" , 'type'=>'STR' ),
                       array('key'=>':id' , 'value'=>$query , 'type'=>'STR' ));
        $data['total'] = $this->getTotal( $sql, $param );
        $data['rows'] =  $this->getRow($sql, $param , $p );
