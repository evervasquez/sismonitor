...
class InstitucionController extends Controller
{
    public function index() 
    {
        ...
        $data['query'] = $_GET['q'];
        $data['pag'] = $this->Pagination(array('rows'=>$data['data']['rowspag'],'url'=>'index.php?controller=Institucion&action=index','query'=>$_GET['q']));
        $view = new View();
        $view->setData($data);
        $view->setTemplate( '../view/Institucion/_Index.php' );
        $view->setLayout( '../template/Layout.php' );
        $view->render();
    }
    
    //***************************
    public function mostrar_instituciones() {
      if (!isset($_GET['p'])){$_GET['p']=1;}
        ...
        $data['query'] = $_GET['q'];
        $data['pag'] = $this->Pagination_1(array('rows'=>$data['data']['rowspag'],'url'=>'index.php?controller=Institucion&action=mostrar_instituciones','query'=>$_GET['q']));
        $view = new View();
        $view->setData($data);
        $view->setTemplate('../view/Institucion/_listar.php');
    ...
    public function edit() 
    {
        ...
        $view = new View();
		$view->setData($data);
        $view->setTemplate( '../view/Institucion/_Form.php' );
        $view->setLayout( '../template/Layout.php' );
        $view->render();
    }
    ...
    public function save(){
        ...
        if ($_POST['escuela_id']=='')
        {
            ...
            if ($p[0])
            {
                header('Location: index.php?controller=Institucion');
            }
            else 
            {
                $data = array();
                $view = new View();
                $data['msg'] = $p[1];
                $data['url'] =  'index.php?controller=Institucion';
                $view->setData($data);
